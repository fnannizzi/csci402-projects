...
void broadcast_waiters_destroy(){
	int returnValue = 0, i = 0;	
	
	/* Yield to allow other thread to acquire the lock and wait on the CV first. */
	for(i = 0; i < 300; i++){ Yield(); }
	
	Printx("Thread 3 is destroying the lock\n",34,1);
	returnValue = DestroyLock(lock);
	if(returnValue == -1){ 
		Printx("Thread 3 not able to destroy the lock\n",38,1);
	}
	
	Printx("Thread 3 is broadcasing using a destroyed lock\n",49,1);
	returnValue = Broadcast(CV, lock);
	if(returnValue == -1){ 
		Printx("Thread 3 not able to broadcast on the CV\n",43,1);
	}
	
	Printx("Thread 3 is destroying the CV\n",32,1);
	returnValue = DestroyCondition(CV);
	if(returnValue == -1){ 
		Printx("Thread 3 not able to destroy the CV\n",38,1);
	}
	
	Printx("Thread 3 is broadcasting using a destroyed CV\n",47,1);
...
main()
{
    ...
    Fork(wait_norm_1, "", 0);
    Fork(wait_norm_2, "", 0);
    Fork(broadcast_waiters_destroy, "", 0);
