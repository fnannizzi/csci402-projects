...
OpenFile::ReadAt(char *into, int numBytes, int position)
{
    ...
    char *buf;
	DEBUG('a', "Checking if numBytes is invalid or position is greater than fileLength.\n");
    if ((numBytes <= 0) || (position >= fileLength)){
    	return 0; 				// check request
    	DEBUG('a', "NumBytes is invalid, or position is greater than fileLength.\n");
    }
    if ((position + numBytes) > fileLength){		
		numBytes = fileLength - position;
	}
    DEBUG('a', "Reading %d bytes at %d, from file of length %d.\n", 	
